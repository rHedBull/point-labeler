...
class AnnotateHandler(SimpleHTTPRequestHandler):
    ...
    def _handle_finalize_instances(self):
        ...
        # Find latest versioned instance files
        latest_ids = sorted(output_dir.glob("instance_ids.v*.npy"))
        latest_meta = sorted(output_dir.glob("instance_metadata.v*.json"))
        if not latest_ids or not latest_meta:
            self._send_json(500, {"error": "Instance files not found after export"})
            return

        instance_labels = np.load(latest_ids[-1])
        with open(latest_meta[-1]) as f:
        ...
        self.app["graph_method"] = graph.get("method", "proximity")
    ...
    def _handle_export_graph(self):
        ...
        # Get graph nodes from adjacency graph
...
def _check_staleness(output_dir, stage):
    """Check if downstream stage outputs are stale relative to upstream."""
    output_dir = Path(output_dir)

    if stage == "instance":
        # Check if patch outputs match current RANSAC labels
        latest_meta = sorted(output_dir.glob("patch_metadata.v*.json"))
        if latest_meta:
            with open(latest_meta[-1]) as f:
                meta = json.load(f)
            print(f"  Stage 2 outputs: v{meta.get('version', '?')} "
                  f"({meta.get('n_patches', '?')} patches)")
        ...
        if latest_meta:
            with open(latest_meta[-1]) as f:
                meta = json.load(f)
            print(f"  Stage 3 outputs: v{meta.get('version', '?')} "
                  f"({meta.get('n_instances', '?')} instances)")

        # Check for stale graph exports
        latest_graph = sorted(output_dir.glob("connectivity_graph.v*.json"))
        if latest_graph:
            with open(latest_graph[-1]) as f:
                graph_meta = json.load(f)
            graph_fp = graph_meta.get("metadata", {}).get("input_fingerprints", {})
            # Compare against current instance outputs
            current_fp = {}
            for prefix in ("instance_ids", "instance_classes"):
                latest = sorted(output_dir.glob(f"{prefix}.v*.npy"))
                if latest:
                    current_fp[latest[-1].name] = labels_fingerprint(np.load(latest[-1]))
            if graph_fp and current_fp and graph_fp != current_fp:
                print("  WARNING: Graph export is stale — instance outputs have changed.")
                print("  Consider re-exporting the graph after review.")
